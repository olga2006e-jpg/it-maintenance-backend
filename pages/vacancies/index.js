...
export class VacanciesPage {
    ...
    showError(errorMessage) {
        ...
        if (container) {
            container.innerHTML = `
                <div class="alert alert-danger w-100 text-center" role="alert" style="background: #f8d7da; border-color: #f5c6cb; color: #721c24;">
                    <strong>Ошибка!</strong> Не удалось загрузить вакансии.
                    <br><small>${errorMessage || 'Проверьте подключение к серверу'}</small>
                </div>
            `;
        }
    }
    ...
    async deleteCard(id) {
        try {
            await api.delete(apiUrls.deleteVacancy(id));
            await this.loadVacanciesFromApi();
        } catch (error) {
    ...
    async addCard() {
        ...
        try {
            await api.post(apiUrls.createVacancy(), newCard);
            await this.loadVacanciesFromApi();
        } catch (error) {
